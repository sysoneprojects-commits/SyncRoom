...
class RoomSocket {
  ...
  Future<void> connect({
    ...
    _channel = WebSocketChannel.connect(
      Uri.parse('$wsBase/api/v1/rooms/$code/ws'),
    );
    ...
    _channel!.stream.listen(
      ...
      onDone: () {
        _events.add({'type': 'socket.closed'});
      },
    );
  ...
  void play(double seconds) =>
      send('sync.play', {'position': seconds});

  void pause(double seconds) =>
      send('sync.pause', {'position': seconds});

  void seek(double seconds) =>
      send('sync.seek', {'position': seconds});
