...
class SubtitleService {
  Future<SubtitleTrack?> pickSrt() async {
    ...
    return SubtitleTrack(name: file.name, cues: parseSrt(raw));
  }
  ...
  List<SubtitleCue> parseSrt(String raw) {
    ...
    final timing = RegExp(r'(\d{2}:\d{2}:\d{2}[,.]\d{3})\s*-->\s*(\d{2}:\d{2}:\d{2}[,.]\d{3})');
    for (final block in blocks) {
      ...
      cues.add(SubtitleCue(start: _duration(match.group(1)!), end: _duration(match.group(2)!), text: text));
    }
